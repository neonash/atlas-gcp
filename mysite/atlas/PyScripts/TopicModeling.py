...
import sys
from atlas.models import Review, Social, Uploads
...
def main(request, num_topics):
    ...
    try:
        if True:
            ...
            #     chunksize = 10 ** 5
            #     chunks = pd.read_csv(filename, chunksize=chunksize, encoding='utf-8')
            #     print(chunks)
            ...
            if is_csv:
                qs = Social.objects.filter(dataset_filename=request).values()
                print("Data in Social")
                if len(qs) == 0:
                    qs = Uploads.objects.filter(pCategory=request).values()
                    print("Data in Uploads")
            else:
                qs = Review.objects.filter(pid_id__pCategory=request).values()
                if len(qs) == 0:
            ...
            texts = map(lambda s: tokeniser(s), [str(l).decode('utf-8', 'ignore') for l in lines])
