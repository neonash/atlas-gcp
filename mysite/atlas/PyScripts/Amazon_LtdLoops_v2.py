...
import urllib
import atlas.scrapers.scrapers.spiders.scrapytest as AmazonProductSpider
import atlas.scrapers.scrapers.spiders.AmazonReviewSpider as AmazonReviewSpider
...
def amazon_all_info(kw_str1):
    global prod_title, rev_url, price, prod_img_src, prod_descr, prod_rating, browser, prod_URL, prods_info, kw_str, all_reviews, all_kw_all_rev, status_code
    kw_str = kw_str1
    ...
    try:
        ...
        try:
            AmazonProductSpider.run_spider(kw_str)
        except:
            print(traceback.print_exc())

        time.sleep(3)

        prod_ids = Product.objects.filter(pCategory=kw_str).values_list('pid', flat=True)
        prod_ids = list(set(prod_ids))
        # print("prod ids = ", prod_ids)
        prod_urls1 = Product.objects.filter(pCategory=kw_str, pid__in=prod_ids).values_list('pURL', flat=True)
        prod_urls1 = list(set(prod_urls1))
        # print("prod urls = ", prod_urls)

        prod_urls_dict = dict()
        for i in prod_ids:
            i1 = i[-10:]  # to extract last 10 characters  - ASIN
            for j in prod_urls1:
                if i1 in j:
                    # print("match found")
                    prod_urls_dict[i] = j
        # print(prod_urls_dict)

        try:
            AmazonReviewSpider.run_spider(prod_urls_dict, kw_str)
        except:
            print(traceback.print_exc())
        print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
        ...
        #
        #             try:
        #                 prod_brand = browser.find_element_by_id("bylineInfo").text
        #             except:
        #                 try:
        #                     prod_brand = browser.find_element_by_id("bylineInfo_feature_div").text
        #                 except:
        #                     prod_brand = '#N/A'
        #
        #             try:
        ...
        #             try:
        #                 elem = browser.find_element_by_xpath("//ul[@class = 'a-pagination']//li[last() - 1]//a")
        ...
        #     row['rid'] = str(row['rid'])
        #     row['rid'] += "_" + str(len(str(row['rText']).encode('utf-8').split(" "))) + "_" + str(len(str(row['rText'])))
        # print("%%%%%%%%%%%%%%%%%%%%%")
        ...
        # #print "Saving the file at... " + full_path
        # # all_kw_all_rev.to_csv(full_path, index=False, encoding='utf-8')
        #
