...
import django
django.setup()
from atlas.models import Product, Review, Analysis, Uploads, UploadAnalyses, DimenMap, TagDicts, TagDictsUpl, Social, SocialAnalyses
...
#
#     final_data_frame1 = []
#     final_prod_data_frame1 = []
#     final_rev_data_frame1 = []
#     status_dict = {}
#
#     time.sleep(0.5)
#
...
#             # print(type(this_dt2))
#
#             if isinstance(this_dt2, basestring):
#                 this_dt2 = datetime.strptime(this_dt2, "%Y-%m-%d")
#
#             # print(this_dt2)
#             dt1 = this_dt2.day
#             mth = this_dt2.month
...
#                                          rUser=row['rUser'], dt=dt1, mth=mth, year=year)
#             #print(row['rTitle'])
#             obj1.save()
#
...
def main2(kw_str, filecontents, tag_dict):
    ...
    print("Adding dimension mapping to db")
    ...
    tag_dict_df = tag_dict_df.drop_duplicates()
    ...
    try:
        ...
        obj1 = DimenMap.objects.create(dict_filename=kw_str, **dict1)
        obj1.save()
    except:
    ...
    kw_str_obj = DimenMap.objects.get(dict_filename=kw_str)
    headers_list2 = tag_dict_df.columns.values.tolist()
    headers_list2 = [h for h in headers_list2 if not h == "ngram"]  # to ignore 'ngram' column
    ...
    try:
        for idx, row in tag_dict_df.iterrows():
            try:
                ...
                obj2 = TagDictsUpl.objects.create(dict_filename=kw_str_obj, ngram=row['ngram'], **dict2)
                obj2.save()
    ...
    unix_dt = datetime.strptime(dt, '%Y-%m-%d')
    ...
    for index, row in final_data_frame1.iterrows():
        ...
        try:
            ...
            print(this_dt2.day, this_dt2.month, this_dt2.year)
            dt1 = this_dt2.day
            mth = this_dt2.month
            year = this_dt2.year

            obj = Uploads.objects.create(rid=db_cols['rid'], rDate=db_cols['rDate'], rDate2=db_cols['rDate'],
                                         rRating=db_cols['rRating'], rText=db_cols['rText'], rTitle=db_cols['rTitle'],
                                         rURL=db_cols['rURL'], rUser=db_cols['rUser'], pCategory=kw_str, dt=dt1, mth=mth, year=year)
            fk_rid_obj = row['rid']
...
# FILE UPLOAD Main function:
def main3(kw_str, filecontents, tag_dict):
    global status_code
    if ".csv" in kw_str:
        kw_str = kw_str[:-4]
    #print("product category :>", kw_str)
    ...
    try:
        for idx, row in tag_dict_df.iterrows():
            try:
                ...
                obj2 = TagDicts.objects.create(dict_filename=kw_str_obj, ngram=row['ngram'], **dict2)
                obj2.save()
            except UnicodeDecodeError:
                pass
            except:
    ...
    try:
        ...
        hl = [field.name for field in Social._meta.get_fields()]


        to_excl = ["dataset_filename", "aid", "socialanalyses", "taggeddata"]
        for h in hl:
            str1 = str(h).split(".")[len(str(h).split(".")) - 1]
            if str1[len(str1) - 1] == ">":  # for foreign key fields
                headers_list3.append(str1[:len(str1) - 1])
            else:
                headers_list3.append(str1)

        #print(headers_list3)
        # to remove 'dataset_filename' cols from loop
        headers_list3 = [h for h in headers_list3 if h not in to_excl]
        ...
        for i, r in filecontents.iterrows():
            for h in headers_list3:
                try:
                    if h in filecontents.columns.values.tolist():
                        ...
                        if h in ['rDate2', 'rDate']:
                            dict3['rDate2'] = datetime.strptime(r[h], "%Y-%m-%d")
                            dict3['rDate'] = dict3['rDate2']
                        else:
                            dict3[h] = r[h]
                except:
                    pass
            # print(dict3)
            try:
                aid = str(kw_str)+str(dict3['rid'])
                # print(str(aid))

                this_dt = dict3['rDate2']
                # print(this_dt)
                # print(type(this_dt))

                if isinstance(this_dt, basestring):
                    this_dt = datetime.strptime(this_dt, "%Y-%m-%d")

                # print(this_dt)
                dict3['dt'] = this_dt.day
                dict3['mth'] = this_dt.month
                dict3['year'] = this_dt.year

                obj3 = Social.objects.create(aid=aid.encode('utf-8'), dataset_filename=kw_str, **dict3)
                obj3.save()
                soc_obj = Social.objects.get(aid=aid.encode('utf-8'))
                obj4 = SocialAnalyses.objects.create(aid=soc_obj, rid=dict3['rid'])
                obj4.save()
                print("obj saved")
            except:
