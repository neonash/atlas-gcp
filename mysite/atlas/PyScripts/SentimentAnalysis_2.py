...
# Main function
def senti_main3(kw_str, filecontents, sentidict):
    global status_code
    ...
    # print("type of contents_df:", type(filecontents))
    filecontents = pd.read_csv(StringIO(filecontents))
    if not 'rRating' in filecontents.columns.values.tolist():
        filecontents['rRating'] = 0
    #print("contents_df: ", filecontents)

    try:
        revs_df = Social.objects.filter(dataset_filename=kw_str).only('rid','rText','rRating').values()
        #print(revs_df)
        for r in revs_df:
            #print(float(r['rRating']))
            curr_rev = r['rText'].encode('utf-8')
            curr_rev1 = clean_rev(curr_rev)
            curr_rating = r['rRating']
            ...
            if final_score < 0 and Decimal(curr_rating) > 2:
                # print "MISMATCH!!! SCORE LESS RATING MORE!!!"
                final_sentiment = "Positive"  # rating takes precedence

            elif final_score > 20 and Decimal(curr_rating) <= 3:
                # print "MISMATCH!!! SCORE MORE RATING LESS!!!"
                final_sentiment = "Negative"  # rating takes precedence
            ...
            try:
                aid = str(kw_str) + str(r['rid'])
                soc_obj = Social.objects.get(aid=aid.encode('utf-8'))
                analysis_obj = SocialAnalyses.objects.get(aid=soc_obj)
                analysis_obj.sentiScore = final_score
                analysis_obj.sentiment = final_sentiment
                analysis_obj.save()
            except:
                # try:
                #     SocialAnalyses.objects.get(aid=aid.encode('utf-8')).update(sentiScore=score, sentiment=final_sentiment)
                # except:
                print("Error while updating analysis table!")
                print(traceback.print_exc())
        status_code = 200
