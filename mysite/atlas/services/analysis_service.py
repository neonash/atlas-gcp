...
import django
django.setup()
from atlas.models import Product, Review, Analysis, Uploads, UploadAnalyses, Social, SocialAnalyses
...
def reloadBrandWithSrc(kw, source):
    print("inside reload brand")
    # print(source)
    sources_file = pd.read_csv(dbConfig.dict['sourcesUrl'], header=0, encoding='utf-8')
    # print(sources_file)
    source1 = []
    for s in source:
        for i, r in sources_file.iterrows():
            print(str(s).lower(), str(r['siteName']).lower())
            if str(s).lower() == str(r['siteName']).lower():
                print(type(r['siteCode']))
                source1.append(str(r['siteCode']))
    # print(source1)
    source = source1
    print("sources found")

    brands = Product.objects.filter(pCategory=kw, siteCode__in=source).distinct().values('pBrand')  # to return dictionary of values for each column
    #brands = Product.objects.filter(pCategory=request).distinct().values_list('pBrand', flat=True)  # to return only values of that column
...
def reloadBrandWithSku(kw, sku):
    print("inside reload brand")

    brands = Product.objects.filter(pCategory=kw, pModel__in=sku).distinct().values('pBrand')  # to return dictionary of values for each column
    #brands = Product.objects.filter(pCategory=request).distinct().values_list('pBrand', flat=True)  # to return only values of that column
...
def getSource(kw, brand):
    sources = Product.objects.filter(pCategory=kw, pBrand__in=brand).distinct().values_list('siteCode',
                                                                                            flat=True)  # to return dictionary of values for each column
    #print(sources)
    sources_file = pd.read_csv(dbConfig.dict['sourcesUrl'], header=0, encoding='utf-8')
    sources1 = []
...
def reloadSourceWithBrd(kw, brand):
    print("inside reload source")
...
def reloadSourceWithSku(kw, sku):
    print("inside reload source")

    sources = Product.objects.filter(pCategory=kw, pModel__in=sku).distinct().values('siteCode')  # to return dictionary of values for each column
    # print(sources)
    sources_file = pd.read_csv(dbConfig.dict['sourcesUrl'], header=0, encoding='utf-8')
    sources1 = []
    for s in list(sources):
...
def reloadSkuWithBrd(kw, brand):
    print("inside reload sku")

    sku = Product.objects.filter(pCategory=kw, pBrand__in=brand).distinct().values('pModel')  # to return dictionary of values for each column
...
def reloadSkuWithSrc(kw, source):
    print("inside reload sku")
    ...
    sku = Product.objects.filter(pCategory=kw, siteCode__in=source).distinct().values('pModel')  # to return dictionary of values for each column
    sku_json = json.dumps(list(sku), cls=DjangoJSONEncoder)
    return sku_json
...
def getCommonTrigChart(kw):
    ...
    if test:
                            ...
                            if trig_drill[1] == trig_drill[0]:  # if main trigger doesnt have subtrigger
                                ...
                                for st in range(len(sub_trigs)):  # check if it is listed and update value
                                    if sub_trigs[st]["id"] == trig_drill[0]:
                                        ...
                                        # st_idx = 0
                                        for st_d in range(len(sub_trigs[st][
                                                                  "data"])):  # this loop to check if entry already exists and increment
                                            if trig_drill[1] == sub_trigs[st]["data"][st_d][0]:  # if main trigger listed and subtrigger also listed
                                                sub_trigs[st]["data"][st_d][1] += 1
                                                st_flag = True
        ...
        data1 = Social.objects.filter(dataset_filename=kw).values_list('aid')
        #data1 = list(data1)
        #print("data1")
        #print(list(data1))

        data2 = SocialAnalyses.objects \
            .filter(aid_id__in=data1) \
            .values('trigger')
                            ...
                            if driv_drill[1] == driv_drill[0]:  # if main driver doesnt have subdriver
                                ...
                                for sd in range(len(sub_drivs)):  # check if it is listed and update value
                                    if sub_drivs[sd]["id"] == driv_drill[0]:
                                        ...
                                        # st_idx = 0
                                        for sd_d in range(len(sub_drivs[sd][
                                                                  "data"])):  # this loop to check if entry already exists and increment
                                            if driv_drill[1] == sub_drivs[sd]["data"][sd_d][0]:  # if main driver listed and subdriver also listed
                                                sub_drivs[sd]["data"][sd_d][1] += 1
                                                sd_flag = True
